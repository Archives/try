...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    {
        // Roll dodge
        int32 dodgeChance = int32(pVictim->GetUnitDodgeChance()*100.0f - skillDiff * 0.04f);
    ...
    {
        // Roll parry
        int32 parryChance = int32(pVictim->GetUnitParryChance()*100.0f  - skillDiff * 0.04f);
        // Reduce parry chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER)
...
float Unit::GetUnitDodgeChance() const
{
    if(hasUnitState(UNIT_STAT_STUNNED))
        return 0.0f;
    if( GetTypeId() == TYPEID_PLAYER )
        return ((Player*)this)->GetDiminishedDodge();
    else
    {
...
float Unit::GetUnitParryChance() const
{
    ...
    if(GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if(player->CanParry() )
        {
            ...
            if(tmpitem)
                chance = ((Player*)this)->GetDiminishedParry();
        }
    }
...
bool Unit::IsImmunedToSpellEffect(SpellEntry const* spellInfo, SpellEffectIndex index) const
{
    ...
    AuraList const& bladestormAuraApply = GetAurasByType(SPELL_AURA_MECHANIC_IMMUNITY_MASK);
    for(AuraList::const_iterator it = bladestormAuraApply.begin(); it != bladestormAuraApply.end(); ++it)
        if (((*it)->GetId() == 46924) &&                                                // Bladestorm Immunity custom handling
    ...
    if(uint32 mechanic = spellInfo->EffectMechanic[index])
    {
        ...
        for(AuraList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
            if ((*iter)->GetModifier()->m_miscvalue & (1 << (mechanic-1)) &&
                ((*iter)->GetId() != 46924))                                               // Bladestorm Immunity already handled
                return true;
    }
...
bool Unit::isIgnoreUnitState(SpellEntry const *spell)
{
    ...
    // Ice Lance
    if(spell->SpellIconID == 186 && spell->SpellFamilyName == SPELLFAMILY_MAGE)
        return true;
    // Shatter
