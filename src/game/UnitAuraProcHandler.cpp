    ...
    switch(dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 62337:
                {
					...
					if (pVictim)
						pVictim->CastCustomSpell(target, triggered_spell_id, &basepoints[EFFECT_INDEX_0], NULL, NULL, true, castItem, triggeredByAura);
					return SPELL_AURA_PROC_OK;
                }
                // Glyph of Life Tap
                case 63320:
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 55680:
                {
                    basepoints[0] = triggerAmount * damage / 200; //10% per tick
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch(dummySpell->Id)
            {
                ...
                {
                    // PPM per victim
                    float ppm = 15.0f; // must be hard-coded + 100% proc chance in DB
                    WeaponAttackType attType = BASE_ATTACK; // TODO: attack type based? 
                    uint32 WeaponSpeed = pVictim->GetAttackTime(attType);
                    float chanceForVictim = pVictim->GetPPMProcChance(WeaponSpeed, ppm);
                    ...
                    pVictim->CastCustomSpell(pVictim, triggered_spell_id, &basepoints[0], NULL, NULL, true, NULL, triggeredByAura);
                    return SPELL_AURA_PROC_OK;
                }
    ...
    switch(scriptId)
    {
        ...
        { 
            ...
            { 
                ...
                default: SPELL_AURA_PROC_FAILED; 
            } 
...
SpellAuraProcResult Unit::HandleAddFlatModifierAuraProc(Unit* pVictim, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 /*cooldown*/)
{
    ...
    switch(spellInfo->SpellFamilyName)
	{
		...
		case SPELLFAMILY_PALADIN:
		{
			...
			if(spellInfo->SpellIconID == 3015)
			{
				// has 3 flat mod auras - proc only first
	...
	switch (dummySpell->SpellFamilyName)
	{
		case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                //Explode Corpse
                case 49555:
					CastSpell(pVictim, 49618, true);
					return SPELL_AURA_PROC_OK;
                //Explode Corpse heroic
                case 59807:
					CastSpell(pVictim,  59809, true);
					return SPELL_AURA_PROC_OK;
			}
			break;
		}
